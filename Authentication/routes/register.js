...
router
    ...
    .post(async(req,res)=>{
        ...
        if(role!=='user' && role !=='admin'){
            ...
            if(hash!=-1){
                let result = await database.executeQuery(`SELECT COUNT(email) AS count FROM users WHERE email = '${email}'`);
                if(result[0].count > 0){
                    res.send({Message:'User already exists'}).status(409).end();
                }else{
                    result = await database.executeQuery(`INSERT INTO users (email, password, role) values ('${email}', '${hash}', '${role}')`);
